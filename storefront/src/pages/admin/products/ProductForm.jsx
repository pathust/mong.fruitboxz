...
export default function ProductForm() {
  ...
  const [form, setForm] = useState({
    ...
    variants: [{ title: "Standard", price: 0, cost_price: 0, packaging_cost: 0, labor_cost: 0, sku: "", id: null }]
  });
    ...
    if (!isNew) {
      ...
      then((d) => {
        ...
        const loadedVariants = p.variants?.map((v) => ({
          ...
          packaging_cost: Number(v.metadata?.packaging_cost ?? 0),
          labor_cost: Number(v.metadata?.labor_cost ?? 0)
        })) || [{ title: "Standard", price: 0, cost_price: 0, packaging_cost: 0, labor_cost: 0, sku: "", id: null }];
  ...
  const addVariant = () => {
    setForm((prev) => ({
      ...prev,
      variants: [...prev.variants, { title: "", price: 0, cost_price: 0, packaging_cost: 0, labor_cost: 0, sku: "", id: null }]
    }));
  };
                ...
                <div key={i} className="flex flex-col gap-3 bg-gray-50 p-4 rounded-xl border border-gray-200 relative group">
                  <div className="flex flex-wrap gap-3">
                    <div className="w-full lg:w-[22%]">
                      <label className="block text-xs font-medium text-gray-500 mb-1">Tên phân loại</label>
                      <input type="text" value={v.title} onChange={(e) => updateVariant(i, 'title', e.target.value)} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" placeholder="VD: Size L" />
                    </div>
                    ...
                    <div className="w-full lg:w-[15%]">
                      <label className="block text-xs font-medium text-gray-500 mb-1">Phí đóng gói (₫)</label>
                      <input type="number" min={0} value={v.packaging_cost} onChange={(e) => updateVariant(i, 'packaging_cost', Number(e.target.value) || 0)} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" />
                    </div>
                    <div className="w-full lg:w-[15%]">
                      <label className="block text-xs font-medium text-gray-500 mb-1">Nhân công (₫)</label>
                      <input type="number" min={0} value={v.labor_cost} onChange={(e) => updateVariant(i, 'labor_cost', Number(e.target.value) || 0)} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" />
                    </div>
                    <div className="flex-1 flex flex-col justify-center px-2">
                      <p className="text-[11px] text-gray-400 leading-tight">SKU và Giá vốn được quản lý tự động qua BOM.</p>
                    </div>
                  </div>

                  {/* Inline Profit Simulator */}
                  <div className="w-full bg-white rounded-lg border border-gray-100 p-3 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 mt-1">
                    <div className="flex flex-wrap items-center gap-4 md:gap-6 text-sm">
                      <div>
                        <p className="text-gray-400 text-[11px] mb-0.5 uppercase tracking-wider font-semibold">Giá bán</p>
                        <p className="font-semibold text-gray-700">{formatVnd(v.price || 0)}</p>
                      </div>
                      <div className="text-gray-300 font-light">-</div>
                      <div>
                        <p className="text-gray-400 text-[11px] mb-0.5 uppercase tracking-wider font-semibold">BOM (Nguyên liệu)</p>
                        <p className="font-semibold text-red-400">{formatVnd(v.cost_price || 0)}</p>
                      </div>
                      <div className="text-gray-300 font-light">-</div>
                      <div>
                        <p className="text-gray-400 text-[11px] mb-0.5 uppercase tracking-wider font-semibold">Đóng gói</p>
                        <p className="font-semibold text-orange-400">{formatVnd(v.packaging_cost || 0)}</p>
                      </div>
                      <div className="text-gray-300 font-light">-</div>
                      <div>
                        <p className="text-gray-400 text-[11px] mb-0.5 uppercase tracking-wider font-semibold">Nhân công</p>
                        <p className="font-semibold text-yellow-500">{formatVnd(v.labor_cost || 0)}</p>
                      </div>
                    </div>

                    <div className="flex items-center gap-4 border-t md:border-t-0 md:border-l border-gray-100 pt-3 md:pt-0 md:pl-5 min-w-[140px] justify-between md:justify-end">
                      {(() => {
                        const totalCost = (v.cost_price || 0) + (v.packaging_cost || 0) + (v.labor_cost || 0);
                        const grossProfit = (v.price || 0) - totalCost;
                        const margin = v.price > 0 ? ((grossProfit / v.price) * 100).toFixed(1) : 0;
                        const isProfit = grossProfit >= 0;
                        return (
                          <>
                            <div>
                              <p className="text-gray-400 text-[11px] mb-0.5 text-right uppercase tracking-wider font-semibold">Lợi nhuận gộp</p>
                              <p className={`font-bold text-base text-right ${isProfit ? 'text-green-600' : 'text-red-500'}`}>{formatVnd(grossProfit)}</p>
                            </div>
                            <div className={`px-2.5 py-1 rounded-md text-xs font-bold ${isProfit ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                              {margin}%
                            </div>
                          </>
                        )
