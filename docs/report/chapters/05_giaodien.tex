...
\begin{longtable}{@{}L{0.25\textwidth} L{0.24\textwidth} L{0.43\textwidth}@{}}
  \toprule
...
Các hình dưới đây được chụp trực tiếp tại \texttt{localhost:5173}. Bộ ảnh được chọn theo hành trình sử dụng thay vì chụp mọi route: sáu màn hình khách hàng bao quát khám phá--cấu hình--tư vấn--checkout và năm màn hình quản trị bao quát KPI--catalog--đơn hàng--tồn kho--cấu hình giao hàng. Ảnh trang chủ được chụp sau khi banner động tải xong; ảnh checkout được chụp sau khi địa chỉ kích hoạt API báo phí. Dữ liệu catalog, giá, ảnh, giỏ hàng và KPI đều đến từ API của dự án, không phải mockup tĩnh.

\begin{figure}[H]
  \centering
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\linewidth]{images/ui-home.png}
    \caption{Trang chủ sau khi banner động tải hoàn tất}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.49\textwidth}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    ...
    \caption{Checkout sau khi tự động cập nhật phí giao hàng}
  \end{subfigure}
...
Card sản phẩm gom ảnh, tên, giá và hành động mua. Trang chi tiết dùng route động theo slug, nhờ đó không cần tạo file riêng cho từng sản phẩm. Khi sản phẩm có nhiều biến thể, người dùng phải chọn biến thể trước khi thêm vào giỏ. Phần dưới trang tập trung vào mô tả, thành phần nguyên liệu và sản phẩm liên quan.
...
Khi chạy thử trên các trang chính, luồng khách hàng khá dễ theo dõi: nút hành động chính dùng màu cam, nội dung bán hàng dùng card và ảnh lớn, chatbot đặt ở góc nên không che checkout. Một vài điểm còn cần chỉnh là chiều cao ảnh sản phẩm, độ tương phản của chữ phụ và cách giải thích nguồn tính phí giao hàng.
...
\begin{longtable}{@{}L{0.27\textwidth} L{0.26\textwidth} L{0.39\textwidth}@{}}
  \toprule
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\linewidth]{images/ui-admin-shipping.png}
  \caption{Cấu hình miễn phí giao hàng, phí mặc định và điểm xuất phát}
  \label{fig:ui-admin-shipping}
\end{figure}

Các bảng quản trị giữ bộ lọc và hành động ở vùng đầu trang, giúp nhân viên chuyển nhanh từ quan sát sang xử lý. Dashboard phù hợp để nhận biết xu hướng; trang đơn hàng cung cấp dữ liệu tác nghiệp; trang tồn kho hỗ trợ quyết định nhập hoặc điều chỉnh hàng. Trang cấu hình giao hàng cho phép điều chỉnh mức phí mặc định, ngưỡng miễn phí, danh sách khu vực miễn phí và tọa độ cửa hàng dùng cho báo phí. Các phần còn nên chỉnh là tên trạng thái tiếng Việt, định dạng tiền và mật độ cột giữa các bảng.

\section{Nhận Xét Về Tính Nhất Quán}

Frontend và admin hiện cùng nằm trong một ứng dụng React nhưng phục vụ hai kiểu người dùng khác nhau. Cách làm này phù hợp với phạm vi đồ án vì giúp phát triển nhanh, nhưng về lâu dài bundle và design system dễ bị phình to. Nếu tiếp tục phát triển, admin có thể được tách thành workspace riêng hoặc chuyển một số phần phù hợp sang Medusa Admin extensions; các thành phần bảng, form, màu và typography cũng nên được gom thành bộ dùng chung.
