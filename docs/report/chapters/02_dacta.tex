...
Chương này mô tả các yêu cầu được rút ra từ mã nguồn hiện tại, thay vì mô tả một hệ thống giả định. Mỗi yêu cầu được gắn với tác nhân, đầu vào, kết quả và trạng thái triển khai. Nhờ đó, từng phần trong báo cáo có thể đối chiếu lại với giao diện, API và dữ liệu trong mã nguồn.
...
\begin{longtable}{@{}L{0.22\textwidth} L{0.25\textwidth} L{0.45\textwidth}@{}}
  ...
  Cơ sở dữ liệu & PostgreSQL 15 + pgvector & Dữ liệu giao dịch, catalog, người dùng, mô-đun tùy biến và hạ tầng vector chuẩn bị cho tìm kiếm/ngữ cảnh ngữ nghĩa. \\[3pt]
...
\begin{longtable}{@{}L{0.10\textwidth} L{0.25\textwidth} L{0.20\textwidth} L{0.37\textwidth}@{}}
  \toprule
  ...
  UC09 & Quản lý catalog, khuyến mãi, tồn kho & Quản lý cửa hàng & Tạo/sửa sản phẩm, danh mục, mã giảm giá, công thức và dữ liệu tồn kho. \\
  UC10 & Quản lý đơn hàng và khách hàng & Quản lý cửa hàng & Tra cứu chi tiết, cập nhật trạng thái xử lý đơn và theo dõi hồ sơ khách hàng. \\
  UC11 & Quản lý nội dung, media và FAQ & Biên tập viên & Quản lý banner, blog, media, FAQ và các trang chính sách. \\
  UC12 & Báo cáo vận hành & Quản lý cửa hàng & Xem chỉ số doanh thu, chi phí, lợi nhuận và dữ liệu tổng hợp. \\
  UC13 & Quản lý người dùng và RBAC & Super Admin & Gán vai trò, quyền và giới hạn chức năng theo mô-đun/hành động. \\
  UC14 & Cấu hình giao hàng và website & Super Admin hoặc tài khoản được cấp quyền \texttt{settings.*} & Cập nhật phí vận chuyển, ngưỡng miễn phí, khu vực phục vụ và cấu hình website. \\
  \bottomrule
\end{longtable}

Hình~\ref{fig:use-case-overview} nhóm các use case theo hai vùng khách hàng và quản trị. Quan hệ kế thừa actor được dùng để tránh lặp đường nối: khách hàng kế thừa khả năng của mọi khách; Super Admin kế thừa phạm vi thao tác của biên tập viên và quản lý cửa hàng, đồng thời có thêm quyền RBAC và cấu hình hệ thống. Đây là sơ đồ tổng quan; các điều kiện và luồng thay thế được đặc tả chi tiết ở các mục tiếp theo.
...
Các sơ đồ trong báo cáo tuân theo quy ước UML được sử dụng trong học phần: tác nhân nằm ngoài biên hệ thống; use case được biểu diễn bằng hình ellipse; quan hệ \textit{include}/\textit{extend} dùng đường nét đứt và mũi tên phụ thuộc; quan hệ kế thừa actor dùng nét đứt với tam giác rỗng hướng về actor tổng quát hơn; lớp có các ngăn tên--thuộc tính; biểu đồ trình tự đọc từ trên xuống; trạng thái chỉ nối bằng các chuyển tiếp có ý nghĩa nghiệp vụ. Màu sắc chỉ dùng để phân vùng hoặc làm tiêu đề, không mã hóa thêm ngữ nghĩa khi chưa có chú giải. Phần trình bày được tách rõ giữa sơ đồ phân tích, ảnh giao diện và sơ đồ triển khai.
...
\begin{longtable}{@{}L{0.23\textwidth}L{0.69\textwidth}@{}}
  ...
  Luồng chính & (1) Xác thực payload; (2) nếu có khóa idempotency thì tìm đơn đã xử lý trong phạm vi khách đang đăng nhập hoặc email khách vãng lai; (3) workflow tải lại biến thể và giá VND; (4) tính nhu cầu BOM, kiểm tra tồn; (5) tính lại ship và promotion tại server; (6) tạo order \texttt{pending} và mã \texttt{MONG-...}; (7) phát \texttt{order.placed}; (8) trả order và bảng tổng hợp. \\
  ...
  Quy tắc tin cậy & Không dùng giá/tổng tiền gửi từ trình duyệt làm nguồn chuẩn. Phí giao hàng và giảm giá được tính lại ở backend; idempotency chỉ có hiệu lực khi client cung cấp khóa và khóa được kiểm tra theo đúng người dùng/email tương ứng. Giao diện checkout hiện chưa sinh khóa này, nhưng API đã có điểm mở rộng để bổ sung. \\
...
Hình~\ref{fig:checkout-activity} mô tả luồng nghiệp vụ từ góc nhìn trách nhiệm; Hình~\ref{fig:shipping-quote-sequence} mô tả bước định vị và báo phí; Hình~\ref{fig:checkout-flow} mô tả các bước kiểm tra khi tạo đơn. Vì vậy, UC05 không chỉ được mô tả bằng bảng mà còn có sơ đồ thể hiện thứ tự xử lý và trách nhiệm giữa frontend, backend và các mô-đun liên quan.

\subsubsection{Bảng quyết định phí giao hàng}

\begin{longtable}{@{}L{0.29\textwidth}L{0.25\textwidth}L{0.38\textwidth}@{}}
  \toprule
...
\subsection{UC14 -- Cấu hình giao hàng và website}

\begin{longtable}{@{}L{0.23\textwidth}L{0.69\textwidth}@{}}
  \toprule
  \textbf{Thuộc tính} & \textbf{Đặc tả} \\
  \midrule
  Tác nhân & Super Admin hoặc tài khoản quản trị được cấp quyền \texttt{settings.*}. \\
  Tiền điều kiện & JWT admin hợp lệ; người dùng có quyền cấu hình tương ứng, đặc biệt là nhóm quyền \texttt{settings}. \\
  Luồng chính & Cập nhật phí giao hàng mặc định, ngưỡng miễn phí, danh sách khu vực miễn phí, tọa độ cửa hàng, nội dung website và các trang chính sách. \\
  Ràng buộc & Các giá trị ảnh hưởng trực tiếp tới checkout như phí giao hàng và tọa độ xuất phát phải được lưu tại backend; frontend chỉ hiển thị và gửi yêu cầu cập nhật. \\
  Hậu điều kiện & API báo phí và luồng checkout sử dụng cấu hình mới trong các lần tính tiếp theo. \\
  \bottomrule
\end{longtable}

\section{Truy Vết Yêu Cầu Sang Thiết Kế}

\begin{longtable}{@{}L{0.15\textwidth}L{0.27\textwidth}L{0.25\textwidth}L{0.25\textwidth}@{}}
  \toprule
  ...
  UC05 & Activity, luồng định vị/báo phí, luồng tạo đơn, state order & Checkout UI/API, Shipping module, Ingredient, Order, Event Bus & Hình~\ref{fig:ui-assist-checkout} \\
  UC08 & Sequence chatbot & Chatbot API, FAQ/catalog retrieval, Groq & Hình~\ref{fig:ui-assist-checkout} \\
  UC09--UC14 & Module, class, ERD và deployment & Admin routes, RBAC, module service & Hình~\ref{fig:ui-admin-overview}, \ref{fig:ui-admin-operations} và \ref{fig:ui-admin-shipping} \\
  \bottomrule
\end{longtable}
...
\begin{longtable}{@{}L{0.22\textwidth} L{0.70\textwidth}@{}}
  ...
  Tin cậy & Checkout hỗ trợ khóa idempotency ở backend; giá được xác minh tại server; cache, Redis và tìm kiếm đều có phương án suy giảm chức năng. \\
...
\begin{itemize}[leftmargin=2em, itemsep=4pt]
  ...
  \item Khóa idempotency của checkout chỉ được đối chiếu trong phạm vi khách đang đăng nhập hoặc email khách vãng lai.
...
Phiên bản được rà soát chưa tích hợp cổng thanh toán trực tuyến; đơn mới được ghi nhận với trạng thái \texttt{not\_paid}. Subscriber email mới ghi nội dung ra log thay vì gửi qua nhà cung cấp thực. Các phần sau bán hàng như gửi thông báo, đối soát thanh toán và chăm sóc đơn vẫn còn ở mức phác thảo, chưa nên xem là chức năng hoàn chỉnh.
