...
Hệ thống sử dụng PostgreSQL 15. Phần lớn bảng thương mại điện tử được Medusa quản lý thông qua module và migration; dự án bổ sung các bảng riêng cho nội dung website, phân quyền, nguyên liệu và yêu cầu mua số lượng lớn. Cấu hình giao hàng được lưu trong nhóm thiết lập website, còn logic gợi ý địa chỉ và tính phí được tách thành module nghiệp vụ riêng. Vì Medusa khuyến khích cô lập mô-đun, quan hệ giữa mô-đun lõi và mô-đun tùy biến được khai báo bằng module link.

Ngoài cơ sở dữ liệu quan hệ, hệ thống còn dùng Redis cho dữ liệu tạm thời, Meilisearch cho chỉ mục tìm kiếm và MinIO cho tệp nhị phân. PostgreSQL được cấu hình thêm pgvector trong môi trường Docker để chuẩn bị cột embedding cho sản phẩm và log chatbot, nhưng luồng tìm kiếm hiện hành vẫn dựa trên Meilisearch và fallback Query API. Các thành phần này không thay thế PostgreSQL mà phục vụ các kiểu truy cập chuyên biệt.
...
\begin{itemize}[leftmargin=2em, itemsep=4pt]
  ...
  \item \texttt{shipping}: không tạo bảng nghiệp vụ riêng trong phạm vi hiện tại; service đọc cấu hình từ \texttt{site\_setting} để gợi ý khu vực, reverse geocode và tính phí giao hàng.
\end{itemize}
...
Checkout chạm tới nhiều loại dữ liệu: giá và catalog, số lượng nguyên liệu, promotion, order và event hậu xử lý. Luồng này hiện được đưa vào Medusa workflow để tách rõ các bước xác minh giá, tồn kho, giao hàng, khuyến mãi và tạo đơn. Workflow cung cấp transaction/execution boundary và compensation cho các step ghi dữ liệu, trong khi PostgreSQL vẫn bao phủ phần ghi giao dịch do module lõi thực hiện. Các tác vụ ngoài giao dịch như cập nhật Meilisearch hoặc gửi email nên đi qua event/outbox và có retry. Khóa idempotency được lưu trong metadata đơn và chỉ đối chiếu trong phạm vi khách hàng/email tương ứng để cùng một request lặp lại không tạo thêm đơn ngoài ý muốn. Với tồn kho cạnh tranh, bước giữ hoặc trừ hàng cần khóa/điều kiện cập nhật nguyên tử, không chỉ đọc số lượng rồi ghi lại.
...
Script khởi tạo \texttt{init\_vector.sql} tạo extension vector, bổ sung cột embedding cho bảng \texttt{product} và \texttt{site\_chatbot\_question\_log}, đồng thời tạo HNSW index theo cosine similarity. Phần này là nền tảng dữ liệu cho gợi ý hoặc truy xuất ngữ nghĩa trong tương lai; báo cáo không xem đây là chức năng tìm kiếm chính đã hoàn thiện.
...
\begin{itemize}[leftmargin=2em, itemsep=4pt]
  ...
  \item Checkout nhiều bước đã được tách thành workflow; nếu bổ sung thanh toán trực tuyến, phần giao dịch và hoàn tác cần được thiết kế chặt hơn.
\end{itemize}
