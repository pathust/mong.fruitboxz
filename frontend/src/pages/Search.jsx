...
export default function Search() {
  ...
  useEffect(() => {
    ...
    const meiliHost = import.meta.env.VITE_MEILI_HOST
    const meiliKey = import.meta.env.VITE_MEILI_SEARCH_KEY

    let fetchPromise;
    if (meiliHost && meiliKey) {
      // Direct MeiliSearch query (bypasses Medusa API for speed)
      fetchPromise = fetch(`${meiliHost}/indexes/products/search`, {
        method: 'POST',
        headers: {
        ...
        if (!res.ok) throw new Error("MeiliSearch direct fetch failed");
        return res.json().then(data => ({ hits: data.hits, mode: 'meilisearch-direct' }));
      })
    } else {
      // Fallback to Medusa API proxy
      fetchPromise = apiFetch(`/store/search?q=${encodeURIComponent(deferredQuery)}&limit=24`, { signal: controller.signal })
    }
