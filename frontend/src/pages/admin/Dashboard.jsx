...
export default function AdminDashboard() {
    ...
    async function load() {
      ...
      if (analytics) {
        setStats({
          ...
          customers: analytics.metrics?.customers || 0,
      ...
      const [products, orders, customersRes] = await Promise.all([
        api("/admin/products?limit=1").catch(() => ({ count: 0 })),
        api("/admin/orders?limit=1").catch(() => ({ count: 0 })),
        api("/admin/customers?limit=1").catch(() => ({ count: 0 })),
      ])
      setStats({
  ...
  const cards = [
    ...
    { label: "Khách hàng", value: stats.customers, icon: Users, link: "/admin/customers" },
