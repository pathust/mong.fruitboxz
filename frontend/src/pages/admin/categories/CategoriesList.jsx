      ...
      <div className="bg-white rounded-2xl border border-[#eadfcd] shadow-sm flex flex-col">
        <div className="p-4 border-b border-[#eadfcd]">
          <AdminListFilters disableSticky={true}
            actions={
            <>
                <Link to="/admin/categories/new" className="admin-button-primary px-5 py-2.5 text-sm flex items-center gap-2 shadow-lg shadow-primary/20">
          <Plus className="w-4 h-4" /> Thêm danh mục</Link>
              </>
            }
            search={query}
            ...
            {filteredCategories.map((c) =>
              <tr key={c.id} className="hover:bg-[#fffaf4]/50 transition-colors align-top">
                <td className="px-5 py-4">
                  {c.metadata?.image ? (
                    <a href={c.metadata.image} target="_blank" rel="noopener noreferrer" className="block w-12 h-12">
                      <img src={c.metadata.image} alt={c.name} className="w-12 h-12 object-cover rounded-md border border-gray-200 hover:opacity-80 transition-opacity" />
                    </a>
                  ) : (
